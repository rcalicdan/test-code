<?php 
    ...
    */
	...
	function asarino_set_taxonomy($post_id) {
        ...
        switch ($_POST['ptype']) {
            case 'ApartmentSale' : $property_status = 'Sprzedaż Apartamentu'; break;
            case 'HouseSale' : $property_status = 'Sprzedaż Domu'; break;
            case 'ApartmentRental' : $property_status = 'Wynajem Apartamentu'; break;
            case 'HouseRental' : $property_status = 'Wynajem Domu'; break;
            case 'CommercialSpaceRental' : $property_status = 'Wynajem Biura'; break;
            case 'CommercialSpaceSale' : $property_status = 'Sprzedaż Biura'; break;
            case 'LotSale' : $property_status = 'Sprzedaż Działki'; break;
            case 'LotRental' : $property_status = 'Wynajem Działki'; break;
        }	

        
        switch ($_POST['market']) {
            case 'Primary' : $property_type = 'Rynek Pierwotny'; break;
            case 'Secondary' : $property_type = 'Rynek Wtórny'; break;
   			 default:  $property_type = 'Rynek Pierwotny'; break;
        }	
   ...
   /*
	...
	if (isset($_POST['check'])) { // action on click
		...
		foreach ($asari_houses['data'] as $asari_house) {	
			...
			if ( ! $asari_house['pictures'][0] || 
			! preg_match('/^https:\/\//', $asari_house['pictures'][0])) continue;
...
*/
	...
	if (isset($_POST['transaction']) && $_POST['transaction']) {
		switch($_POST['transaction']) {
			case 'sprzedaż' : $transaction = 131; break;
			case 'wynajem' : $transaction = 132; break;
